...
function sendToRenderer(event: ServerEvent) {
  ...
  if (sessionId && remoteManager.isRemoteSession(sessionId)) {
    ...
    // trace.update reserved; currently trace.step is the primary mechanism
...
app
  ...
  .then(async () => {
    ...
    // Start when remote control is enabled
...
async function handleClientEvent(event: ClientEvent): Promise<unknown> {
  ...
  if (event.type === 'session.start' && !configStore.hasUsableCredentialsForActiveSet()) {
    sendToRenderer({
      type: 'error',
      payload: {
        message: 'The current config set has no usable credentials. Please configure them in API settings first.',
