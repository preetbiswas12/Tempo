...
${hints.join('\n')}
  ...
  async run(session: Session, prompt: string, existingMessages: Message[]): Promise<void> {
    ...
    try {
      // The SDK attaches many listeners to the same AbortSignal; raise the limit to avoid spurious warnings during debugging.
      setMaxListeners(0, controller.signal);
    } catch {
      // Older runtimes may not support adjusting the listener limit on EventTarget — ignore.
    }
...
- Use WebSearch/WebFetch only when Chrome MCP is unavailable or the user explicitly asks for generic web search.
      ...
      if (controller.signal.aborted && abortedByTimeout) {
        ...
        const errorMsg: Message = {
          ...
          sessionId: session.id,
          role: 'assistant',
          content: [{ type: 'text', text: '**Request timed out**: No response received for a long time; the operation has been aborted.' }],
          timestamp: Date.now(),
        };
