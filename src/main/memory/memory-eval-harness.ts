...
const DEFAULT_EVAL_CASES: MemoryEvalCase[] = [
  {
    ...
    messages: [
      { role: 'user', text: 'Please answer in Chinese by default from now on.', timestamp: 1 },
      { role: 'assistant', text: 'OK, I will use Chinese by default.', timestamp: 2 },
      {
        role: 'user',
        text: 'Implement gateway token rotation in workspace A, and record the synchronization constraints for the remote gateway.',
        timestamp: 3,
      },
      {
        role: 'assistant',
        text: 'Gateway token rotation is complete, and the remote gateway needs to be updated accordingly.',
        timestamp: 4,
      },
    ],
    queries: [
      {
        id: 'query-a1',
        prompt: 'Continue with gateway token rotation, remind me of the key constraints from last time.',
        workspace: '/eval/workspace-a',
        expectedHits: ['gateway token rotation', 'remote gateway', 'source=/eval/workspace-a'],
      },
      {
        id: 'query-a2',
        prompt: 'What is my preferred response language?',
        expectedHits: ['Chinese'],
      },
    ],
  },
  {
    ...
    messages: [
      ...
      {
        role: 'assistant',
        text: 'Noted: refunded and cancelled represent different financial semantics and need to remain as separate states.',
        timestamp: 11,
      },
    ],
    queries: [
      {
        id: 'query-b1',
        prompt: 'Why can\'t refunded and cancelled be merged?',
        workspace: '/eval/workspace-b',
