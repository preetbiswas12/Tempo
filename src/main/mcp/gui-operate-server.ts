...
function isDescriptionDockRelated(description: string): boolean {
  ...
  return /dock|bottom bar|dock bar|bottom dock/.test(normalized);
}
...
function isLikelyAppLaunchVerification(question: string): boolean {
  ...
  const mentionsMenuLike = /(menu|menu bar|popup|panel|widget|dropdown)/i.test(normalized);
  ...
  try {
    ...
    if (/Accessibility privileges not enabled/i.test(result.stderr || '')) {
      const hint =
        ...
        '- Restart Open Cowork after granting permission\n';
      throw new Error(
...
**Note**: The image may have yellow circle markers indicating previously clicked positions (for relative position reference only — they are not necessarily the correct click positions). The marker format is "#number" and the normalized "[y,x]" coordinates. These markers are not part of the interface. Please ignore them and locate only the actual UI element.

Coordinate format: normalized to 0-1000, format is [ymin, xmin, ymax, xmax]
