    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (shouldReconnectOnStructuredToolError(toolErrorMessage)) {
          // Some MCP services wrap connection errors in structured results instead of throwing directly; convert to an error here to reuse the unified reconnection logic.
