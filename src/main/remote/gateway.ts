...
export class RemoteGateway extends EventEmitter {
    ...
    if (!authorized) {
      ...
      if (this.config.auth.mode === 'pairing') {
        ...
        await this.sendToChannel({
          ...
          content: {
            type: 'text',
            text: '⚠️ You do not have permission to use this bot. Please contact the administrator for access.',
          },
          replyTo: message.id,
        });
      }
  ...
  private async handlePairingRequest(message: RemoteMessage): Promise<void> {
    ...
    if (this.pairingRequests.has(userKey)) {
      ...
      if (Date.now() > existing.expiresAt) {
        ...
        await this.sendToChannel({
          ...
          content: {
            type: 'text',
            text: `Please enter the pairing code for verification.\n\nYour pairing code is: **${existing.code}**\n\nSend this pairing code to the administrator for confirmation, or reply with the code directly to complete pairing.`,
          },
          replyTo: message.id,
        });
        return;
    ...
    await this.sendToChannel({
      ...
      content: {
        type: 'text',
        text: `👋 Hello! First-time use requires pairing verification.\n\nYour pairing code is: **${code}**\n\nPlease send this pairing code to the administrator for confirmation. The code is valid for 10 minutes.`,
      },
      replyTo: message.id,
    });
