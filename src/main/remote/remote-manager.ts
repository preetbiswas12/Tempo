/**
 * Remote Manager
 * Remote control system manager, integrating Gateway, Channels, and MessageRouter
 */
...
export class RemoteManager extends EventEmitter {
  ...
  // Default working directory for remote sessions (used when cwd is not specified)
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Set the remote default working directory (prefer config, fall back to global default)
  ...
  async handleQuestionRequest(
    ...
    questions.forEach((q, _qIdx) => {
      ...
      if (q.options && q.options.length > 0) {
        ...
        messageText += '\n';
        if (q.multiSelect) {
          messageText += `*(Multiple choice, separate with commas, e.g.: 1,3)*\n\n`;
        } else {
  ...
  async handlePermissionRequest(
    ...
    let messageText = '⚠️ **Your authorization is required**\n\n';
    messageText += `Tool: **${toolName}**\n\n`;
    messageText += `Input:\n\`\`\`json\n${JSON.stringify(input, null, 2)}\n\`\`\`\n\n`;
    messageText += `---\n`;
    messageText += `Reply "allow" or "y" to authorize\n`;
    messageText += `Reply "deny" or "n" to deny\n`;
  ...
  /**
   * Send remote user message to the local UI (remote sessions only)
   */
