...
export class ToolExecutor {
  ...
  private assertInsideMount(targetPath: string, mounts: MountedPath[]): string {
    ...
    // Reject UNC paths immediately — they reference network shares and can
    // never be inside a local mount.  Checking fs.existsSync / realpathSync on
    // a UNC path triggers a network lookup that can hang indefinitely.
    if (isUncPath(normalizedTarget)) {
