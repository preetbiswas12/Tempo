      ...
      return {
        text: JSON.stringify({
          session_summary: isWorkspaceA
            ? 'Gateway token rotation experience in workspace A'
            : 'Other experience in workspace B',
          ...
          chunks: [
            {
              ...
              details: isWorkspaceA
                ? 'Completed gateway token rotation in workspace A, with follow-up notes preserved.'
  ...
  it('simulates multi-session recall across same and different workspaces', async () => {
    ...
    await service.enqueueIngestion({
      ...
      messages: makeMessages('b-1', [
        ...
        { role: 'assistant', text: 'Noted.', timestamp: 6 },
      ]),
    });
