  ...
  async complete(request: MemoryCompletionRequest): Promise<{ text: string }> {
    ...
    if (
      ...
      if (transcript.includes('gateway token rotation')) {
        return {
          text: JSON.stringify({
            session_summary: 'Record gateway token rotation and remote gateway constraints',
            session_keywords: ['gateway', 'rotation'],
            chunks: [
              {
                summary: 'Implementation and constraints of gateway token rotation',
                details: 'The remote gateway must be synchronized simultaneously to avoid state inconsistency.',
      ...
      return {
        text: JSON.stringify({
          session_summary: 'Record order state machine design constraints',
