...
export function SettingsMemory() {
  ...
  const handleRebuildAll = async () => {
    if (!window.confirm(t('memory.rebuildAllConfirm', 'This will clear and rebuild all memories. Continue?'))) {
      return;
    }
    setIsBusy(true);
    setStatus(null);
    try {
      ...
      setStatus(
        t('memory.rebuildAllSuccess', {
          defaultValue: `All memories rebuilt: ${result.sessionCount} sessions, ${result.workspaceCount} source workspaces`,
    ...
    <div className="space-y-6">
      ...
      <SettingsContentSection
        ...
        description={t(
          ...
          'Inherits from the active API configuration by default. Mainly adjust navigation depth, embedding, and storage directory here.'
        )}
      >
        <div className="space-y-4 rounded-xl border border-border-muted bg-background-secondary/60 p-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <LabeledField label={t('memory.maxNavSteps', 'Max Navigation Steps')}>
              <input
                type="number"
                min={0}
                max={4}
                ...
                {selected ? (
                  <div className="mt-3 space-y-3">
                    ...
                    {selected.sessionId && (
                      ...
                      >
                        {t('memory.inspectSession', 'View this session\'s full memory')}
                      </button>
                    )}
                    {selected.details && (
              ...
              <div className="rounded-xl border border-border-muted bg-background/80 p-4">
                ...
                {inspectedSession ? (
                  ...
                  <p className="mt-3 text-sm text-text-muted">
                    {t('memory.inspectSessionHint', 'Select a session or chunk from the search results above to view')}
                  </p>
                )}
              </div>
            </div>
      ...
      <SettingsContentSection
        ...
        description={t(
          ...
          'View the actual persisted core / unified experience / session_state / eval artifacts directly.'
        )}
      >
      ...
      >
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium text-text-primary disabled:cursor-not-allowed disabled:opacity-60"
          >
            {t('memory.rebuildAll', 'Rebuild All Memories')}
          </button>
          <button
            onClick={() => {
