...
describe('resolveMessageEndPayload', () => {
  ...
  it('surfaces user-facing error text when message_end stops with error', () => {
    ...
    expect(result.errorText).toBe(
      'Model response timed out: no response received from the upstream for a long time. Please try again later or check the current model/gateway load.'
    );
  });
  ...
  it('surfaces empty_success_result when message_end has no content and no streamed fallback', () => {
    ...
    expect(result.effectiveContent).toEqual([]);
    expect(result.errorText).toBe(
      'The model returned an empty success result. The current model or gateway may have compatibility issues. Please try again or switch protocols.'
    );
  });
});
...
describe('buildTerminalErrorMessage', () => {
  ...
  it('uses the retry hint for non-4xx terminal errors', () => {
    ...
    expect(result).toContain('Agent is automatically retrying, please wait...');
  });
});
...
describe('buildTerminalErrorEmissionDetails', () => {
  ...
  it('omits empty flush fragments cleanly', () => {
    ...
    expect(result.messageText).toContain('Agent is automatically retrying, please wait...');
  });
});
