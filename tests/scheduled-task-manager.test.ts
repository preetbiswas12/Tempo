...
describe('ScheduledTaskManager', () => {
  ...
  it('normalizes provided title with schedule prefix', () => {
    ...
    const created = manager.create({
      title: '  Need to summarize papers  ',
      prompt: '  Help me organize today\'s team todos  ',
      cwd: '/tmp/project',
      runAt: now + 60 * 1000,
      enabled: true,
    });

    expect(created.title).toBe(buildScheduledTaskTitle('Need to summarize papers'));
  });
  ...
  it('keeps existing title when prompt changes without explicit title update', () => {
    ...
    const updated = manager.update('title-update', { prompt: 'Summarize weekly sales data and send to group' });
